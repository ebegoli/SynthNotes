...
def randomDate(start, end, prop):
    """ Returns a random date between two time points. """
...
header = Template("$date and $time")

# S - Subjective section
# Choose these from the papers
subjective = Template("Subjective: $symptoms $course")
...
sleep_log_value = random.choice(["irregular","regular","poor"])
cooperativeness = random.choice(["generally","not","greatly"])
behavior_value = random.choice(["good","poor","problematic","satisfactory"])

staff_observations = Template("Staff observations: Patient is experiencing $agitation agitation, a sleep is $sleep_log, \
patient is ${cooperation} cooperative, and the behavior is $behavior.")

staff_observations.substitute(agitation=agitation_value,sleep_log=sleep_log_value, cooperation=cooperativeness, behavior=behavior_value)


# TODO: research common medications and frequency
medications = Template("Medications: ${name, dose, route, frequency, duration, therapeutic and side effects, compliance}")

# TODO: research other treatments and frequency
other_treatments = Template("Other treatments: ${ECT (treatment #, and as with medications)}$, $psychotherapy")

#This will be used as a static sub-header
objective = "Objective:"


appearance = Template("Appearance:" ="$gait ____, $posture ____, ${clothes ____}, $grooming ,$mileage")

general_behavior = Template("General behavior: mannerisms ____, gestures ____, psychomotor activity ____, expression ____," + ",eye contact,able to follow commands/requests,compulsions")

#Write as Patient attitude is .... Include maybe more than one.
attitude = Template("Attitude is "," cooperative, "," hostile, "," defensive, "," open, "," secretive, "," evasive, "," suspicious, "," apathetic," + "easily distracted,focused, etc.")

LOC = Template("LOC: "," vigilant, "," alert, "," drowsy, "," lethargic, "," stuporous, "," asleep, "," comatose, "," confused, "," fluctuating, etc.")

attention_value = ["can attend","can concentrate","distractible","digit span","DLROW","calculations"]
attention = Template( "Attention: $attention")

orientation = Template("Orientation:  $person, $place, $time, $situation")
...
speech = "Speech: volume/tone (loud,soft,monotone,weak,strong), rate (fast,slow,normal,pressured),"+"amount (talkative,spontaneous,expansive,paucity,poverty), fluency/rhythm (slurred,clear, with appropriately placed inflections, "," hesitant, "," with good articulation, "," aphasic)"
...
affect = "Affect (observed): "," appropriate to situation, "," consistent with mood, "," congruent with thought content, fluctuations (labile,even), range (broad,restricted), intensity (blunted,flat, normal), quality ("," sad, "," angry, "," hostile, "," indifferent, "," euthymic, "," dysphoric, "," detached, "," elated, euphoric, "," anxious, "," animated, "," irritable)"
thought_process ="Thought process is $thought_process."
thought_process = "Thought process/form: "," linear, "," goal-directed, "," circumstantial, "," tangential, "," loose associations,incoherent,evasive,racing,thought blocking,perseveration,neologisms"
